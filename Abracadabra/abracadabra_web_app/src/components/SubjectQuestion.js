...
export default function Question({ question }) {
  ...
  const UpdateVotesQuestion = () => {
    QuestionService.GetQuestion(question.id)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    if (tokenExist) {
      ...
      VotesService.GetQuestionVote(question.id)
        .then((res) => {
  ...
  const submitPost = () => {
    VotesService.PostVoteQuestion(vote)
      .then((res) => {
        setError(null);
        setVoted(true);
        UpdateVotesQuestion();
  ...
  const handleVoteDelete = () => {
    VotesService.DeleteVoteQuestion(question.id)
      .then((res) => {
        setError(null);
        setVoted(false);
        setRendered(false);
  ...
  const handleVotePut = () => {
    VotesService.PutVoteQuestion(vote)
      .then((res) => {
        setError(null);
        UpdateVotesQuestion();
      })
      ...
      <div>
        {(() => {
          if (isloggedin == true) {
            ...
            return (
              <a href="/loginpage">
                <div href="/registerpage">
                  <FontAwesomeIcon
                    className="votingArrowDisabled"
                    icon={faChevronUp}
                  />
                </div>
              </a>
        ...
        try {
          ...
          let text =
            err.response.status == 400
              ...
              ? "You need to be logged in to report a question!"
              : "Oops! couldn't reach the report API, try again later.";
    ...
    <>
      <div className="questionHead mx-auto">
        <div className="questionHeadDiv">
          <h1>Q</h1>
          <FontAwesomeIcon
            className="flagIcon WhiteLinks"
            icon={faFlag}
            onClick={HandleQuestionFlagClick}
          />
        </div>
        <Row>
          <Col md={11} className="mx-auto">
            {error && <h6 className="errorVoting"> {error} </h6>}
            <Row>
              <Col md={9}>
